  ...
  const saveCotizacion = async (cotizacionData: Cotizacion, lineasCotizacion: Linea_Cotizacion[]): Promise<any> => {
    ...
    try {
      ...
      if (!cotizacionPayload.vehiculo_id) {
        console.error("Falta vehiculo_id. Vehículo:", cotizacionData.vehiculo, "LocalStorage:", vehicleLocalStorage);
        throw new Error("ID del vehículo es requerido");
      }
      if (!cotizacionPayload.configuracionLocalidad_id) {
        console.error("Falta configuracionLocalidad_id. Config Localidad:", cotizacionData.ConfigLocalidad);
        throw new Error("ID de configuración de localidad es requerido");
      }
      if (!cotizacionPayload.configuracionEdad_id) {
        console.error("Falta configuracionEdad_id. Config Edad:", cotizacionData.ConfigEdad);
        throw new Error("ID de configuración de edad es requerido");
      }
      if (!cotizacionPayload.configuracionAntiguedad_id) {
        console.error("Falta configuracionAntiguedad_id. Config Antigüedad:", cotizacionData.configuracionAntiguedad);
      ...
      console.log("Creando líneas de cotización para cotización ID:", cotizacion_id);
