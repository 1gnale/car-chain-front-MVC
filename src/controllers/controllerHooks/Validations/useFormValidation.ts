...
const useFormValidation = (): UseFormValidationReturn => {
  ...
  const patterns = {
    matricula: /^([A-Z]{2}\d{3}[A-Z]{2}|[A-Z]{3}\d{3})$/i,
    ...
    switch (fieldName) {
      case "matricula":
        ...
        if (!matriculaValue.trim()) {
          ...
          newErrors.matricula =
            "Formato de matrícula inválido (ej: ABC123 O AA111BB)";
        } else {
      ...
      case "numeroMotor":
        ...
        if (!numeroMotorValue.trim()) {
          ...
          newErrors.numeroMotor =
            "El número de motor debe tener entre 6 y 20 caracteres alfanuméricos";
        } else {
      ...
      case "anio":
        ...
        if (!anioValue.trim()) {
          ...
          if (year < 1900 || year > currentYear + 1) {
            newErrors.anio = `El año debe estar entre 1900 y ${
              currentYear + 1
            }`;
          } else {
