...
interface ErrorConfigAntiguedad {
  nombre?: string;
  minima?: string;
  maxima?: string;
  descuento?: string;
...
interface UseFormValidationReturn {
  errors: ErrorConfigAntiguedad;
  isValid: boolean;
  validateField: (
    fieldName: keyof ErrorConfigAntiguedad,
    value: string | boolean
  ) => void;
  validateForm: (formData: ErrorConfigAntiguedad) => boolean;
  clearErrors: () => void;
  clearFieldError: (fieldName: keyof ErrorConfigAntiguedad) => void;
}

const useFormValidationConfigAntiguedad = (): UseFormValidationReturn => {
  const [errors, setErrors] = useState<ErrorConfigAntiguedad>({});
  ...
  const validateField = (
    ...
    switch (fieldName) {
      ...
      case "minima":
        if (!(value as string).trim()) {
          newErrors.minima = "La Antiguedad mínima es requerida";
        } else if (!patterns.minima.test(value as string)) {
          newErrors.minima = "La Antiguedad mínima debe ser mayor a 0";
        } else {
      ...
      case "maxima":
        if (!(value as string).trim()) {
          newErrors.maxima = "La Antiguedad máxima es requerida";
        } else if (!patterns.maxima.test(value as string)) {
          newErrors.maxima = "La Antiguedad máxima debe ser mayor a 0";
        } else {
  ...
  const validateForm = (formData: ErrorConfigAntiguedad): boolean => {
    const newErrors: ErrorConfigAntiguedad = {};
  ...
  const clearFieldError = (fieldName: keyof ErrorConfigAntiguedad): void => {
    const newErrors = { ...errors };
...
export default useFormValidationConfigAntiguedad;
